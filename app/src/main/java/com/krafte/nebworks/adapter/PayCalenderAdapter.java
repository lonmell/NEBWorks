...
public class PayCalenderAdapter extends RecyclerView.Adapter<PayCalenderAdapter.ViewHolder> {
    private static final String TAG = "PayCalenderAdapter";
    ...
    PaylistAdapter mAdapter1;
    PaylistAdapter mAdapter2;
    PaylistAdapter mAdapter3;
    PaylistAdapter mAdapter4;
    PaylistAdapter mAdapter5;
    PaylistAdapter mAdapter6;
    PaylistAdapter mAdapter7;
    ArrayList<CalendarPayData.CalendarPayData_list> caltaskdata1;
    ArrayList<CalendarPayData.CalendarPayData_list> caltaskdata2;
    ArrayList<CalendarPayData.CalendarPayData_list> caltaskdata3;
    ArrayList<CalendarPayData.CalendarPayData_list> caltaskdata4;
    ArrayList<CalendarPayData.CalendarPayData_list> caltaskdata5;
    ArrayList<CalendarPayData.CalendarPayData_list> caltaskdata6;
    ArrayList<CalendarPayData.CalendarPayData_list> caltaskdata7;

    public PayCalenderAdapter(Context context, ArrayList<WorkCalenderData.WorkCalenderData_list> data, ArrayList<CalendarSetStatusData.CalendarSetStatusData_list> data2
    ...
    @NonNull
    @Override
    public PayCalenderAdapter.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        Context context = parent.getContext();
        LayoutInflater inflater = (LayoutInflater) context.getSystemService(Context.LAYOUT_INFLATER_SERVICE);
        View view = inflater.inflate(R.layout.work_calender_item, parent, false);
        PayCalenderAdapter.ViewHolder vh = new PayCalenderAdapter.ViewHolder(view);
    ...
    @Override
    public void onBindViewHolder(@NonNull PayCalenderAdapter.ViewHolder holder, int position) {
                ...
                for (int sun = 0; sun < mData2.size(); sun++) {
                    if (item.getSun().equals(mData2.get(sun).getDay().length() == 1?"0"+mData2.get(sun).getDay():mData2.get(sun).getDay())) {
                        ...
                        for (int i3 = 0; i3 < Response.length(); i3++) {
                            ...
                            if(jsonObject.getString("user_id").equals("holiday")){
...
//            work_state = itemView.findViewById(R.id.work_state);
    ...
    // 리스너 객체 참조를 저장하는 변수
    private PayCalenderAdapter.OnItemClickListener mListener = null;

    // OnItemClickListener 리스너 객체 참조를 어댑터에 전달하는 메서드
    public void setOnItemClickListener(PayCalenderAdapter.OnItemClickListener listener) {
